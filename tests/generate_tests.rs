    ...
    for entry in fs::read_dir(test_data_dir).expect("Failed to read data dir") {
        ...
        if test_file_path.is_file() {
            ...
            let out_file_contents =
                fs::read_to_string(&out_file).expect("Failed to read from gen.rs");
            ...
            let expected_output = fs::read_to_string(&expected_output_file)
                .expect("Failed to read from expected file");

            assert_eq!(out_file_contents.trim(), expected_output.trim());
        }
    }
}
